...
import { getPersistedOrders } from "@/utils/persistedSiteState";
...
const initialOrdersState = getPersistedOrders();

export const useOrdersStore = create<OrdersState>()((set, get) => ({
  orders: initialOrdersState?.orders ?? [],
  nextSequenceNumber: initialOrdersState?.nextSequenceNumber ?? 1,
  createOrder: (input) => {
        ...
        try {
          ...
          await supabase
            ...
            .upsert(
              {
                id: "main",
                next_order_seq: sequenceNumber + 1,
    ...
    try {
      ...
      const { error: settingError } = await supabase
        ...
        .upsert(
          {
            id: "main",
            next_order_seq: nextSequenceNumber,
            updated_at: new Date().toISOString(),
          },
          { onConflict: "id" },
        );
  ...
  syncFromDatabase: async () => {
    try {
      ...
      const mappedOrders: CustomerOrder[] = (ordersData ?? []).map((row) => ({
        ...
        notes: row.notes ?? "",
        bundleFeatures: Array.isArray(row.bundle_features)
          ? row.bundle_features
          : [],
